...
title: "CapstoneED"
description: >
  A gamified peer-assessment platform designed to make continuous feedback simpler and friction
  free, while providing early warnings for lecturers.
date: 2017-08-02
type_label: "Dissertation"
...
Capstone projects are long-term group projects that Computer Science students
have to complete in their final year of studies. CapstoneED is a peer-assessment
platform that aims to assist both students and lecturers in such projects. In
this page I will go over some of the core ideas for the system, but there is so
much more (in fact 2 dissertations worth of more) that I cannot cover everything
here.

Students get frequent requests to complete gamified, fast activities in order to
provide information about their experience with the project. Lecturers on the
other hand get aggregate views from all their students/teams which allows them
...
{{< note >}}As of December 2025 I am in the process of bringing the project into Angular 20+{{< /note >}}
...
coming up. In addition to that, having a single point of evaluation/reflection
is extremely prone to [recency bias](https://en.wikipedia.org/wiki/Recency_bias).
...
participating in the CapstoneED system, with the assessments and the feedback we
have already briefly mentioned, but also through activities such as logging your
work on your project.

![](game-progress.png)

{{< figcaption >}}Example game progress for the above scenario{{< /figcaption >}}
...
{{< figcaption >}}Example project status{{< /figcaption >}}
...
member to a color where red is the minimum assignable value and green is the maximum.
The zones that are assignable are configured by the lecturer.

![](peer-assessment1.png)

{{< figcaption >}}A range type question{{< /figcaption >}}

{{< note >}}I am personally proud of the WizardComponent (aka Stepper) shown here as it was completely implemented from scratch, with a very dynamic API making use of Angular's content projection.{{< /note >}}

Next type of possible questions is the rank type question. In the range type it
is possible to give everyone the same value (i.e. everyone did an excellent job).
In ranking however, a distinct choice must be made for each member — which is
also why we recommend sparing use of this type.

![](peer-assessment2.png)

{{< figcaption >}}A rank type question{{< /figcaption >}}

The final type is a simple feedback form where the student could leave specific
comments for each team member.

![](peer-assessment3.png)

{{< figcaption >}}A feedback type question{{< /figcaption >}}
...
<div class="photo-sides">
  <img style="align-self: start" src="new-log.png" alt="New log entry on desktop">
  <img src="new-log-mobile.png" alt="New log entry on mobile">
</div>

{{< figcaption >}}Creating a new log entry — desktop and mobile{{< /figcaption >}}
