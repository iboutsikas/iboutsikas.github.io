...
export class GestureController {
  ...
  constructor(private config: IConfigProvider) {
    this.position$ = this.positionSubject
      .asObservable()
  ...
  public connect(element: HTMLElement): void {
    ...
    const end$ = pointerUp$.pipe(map((e) => createGestureEvent('end', this.getPointerPos(e))));
        ...
        exhaustMap((startEvent) => {
          ...
          return merge(
            move$.pipe(
              filter(
                (moveEvent) =>
                  CoverMath.distanceSq(lastEvent.position, moveEvent.position) >= threshold2
              )
            ),
            end$
          ).pipe(
