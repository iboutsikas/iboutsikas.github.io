...
To create an about page, make sure `layout` is set to `about`, and that the `author` key is set to an author defined in `_data/authors.yml`. For more on authors, see [Adding an author]({{ site.baseurl }}{% link docs/6.4.1/configuration.md %}#adding-an-author).
...
Continue with [Writing]({{ site.baseurl }}{% link docs/6.4.1/writing.md %}){:.heading data-flip="title"}
{:.read-more}
